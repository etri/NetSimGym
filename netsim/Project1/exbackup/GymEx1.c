...
void execute_SINR_command(ptrCLIENTINFO info, ptrCOMMANDARRAY command, int index)
{
    ...
    if (sinr_found)
    {
        ...
        contain_obs(info, "SINR", values, 1);
    }
    else
    {
...
void execute_Throughput_command(ptrCLIENTINFO info, ptrCOMMANDARRAY command, int index)
{
    ...
    while (assocInfo)
    {
        if (assocInfo->ueId == ueId && assocInfo->isAssociated)
        {
            ...
            if (phy->ca_count > 0)
            {
                ...
                if (layerCount > 0)
                {
                    throughput_value = (pdcp->TotalPDSCHBytes * 8) / (3 * frameDuration);
                    fprintf(stderr, "Throughput = %f bps\n", throughput_value);
    ...
    if (throughput_found)
    {
        float values[SIZE_OF_MAP];
        for (uint32_t i = 0; i < SIZE_OF_MAP; i++) {
            values[i] = throughput_value; // Random values between 0 and 99
        }
...
void execute_SumThroughput_command(ptrCLIENTINFO info, ptrCOMMANDARRAY command, int index)
{
    ...
    // 주어진 gNB ID 그룹에 대해 throughput을 계산하는 부분을 시나리오별로 직접 구현
        ...
        for (NETSIM_ID j = 0; j < NETWORK->ppstruDeviceList[gnbId - 1]->nNumOfInterface; j++)
        {
            ...
            while (assocInfo)
            {
                if (assocInfo->isAssociated)
                {
                    ...
                    {
                        double throughput_value = (pdcp->TotalPDSCHBytes * 8) / (3 * frameDuration);
    ...
    // 시나리오 2 그룹 throughput 계산
    if (fn_NetSim_Stack_GetDeviceId_asName("Macro cell eNB Omni Ant_13") != NULL)  // gNB ID 13이 존재하는지 확인
    {
    ...
    if (scenario5_throughput > 0)
    {
        ...
        contain_reward(info, "SumThroughput", values, 5);
    }
    else if (scenario4_throughput > 0)
    {
        ...
        contain_reward(info, "SumThroughput", values, 4);
    }
    else if (scenario3_throughput > 0)
    {
        ...
        contain_reward(info, "SumThroughput", values, 3);
    }
    else if (scenario2_throughput > 0)
    {
        ...
        contain_reward(info, "SumThroughput", values, 2);
    }
    else
    {
...
void execute_SumThroughput_command(ptrCLIENTINFO info, ptrCOMMANDARRAY command, int index)
{
    ...
    if (throughput_found)
    {
        fprintf(stderr, "Total Sum Throughput for all gNBs: %f bps\n", sum_throughput);
        float values[SIZE_OF_MAP];
        for (uint32_t i = 0; i < SIZE_OF_MAP; i++) {
    ...
    if (actmsg->command_name == 1) {
        ...
        if (actionCount >= 1) {
            ...
            for (NETSIM_ID i = 0; i < NETWORK->nDeviceCount; i++) {
                for (NETSIM_ID j = 0; j < NETWORK->ppstruDeviceList[i]->nNumOfInterface; j++) {
                    if (!isLTE_NRInterface(i + 1, j + 1) || !isGNB(i + 1, j + 1)) {
                        //fprintf(stderr, "Device %d, Interface %d is not LTE NR or not a gNB.\n", i + 1, j + 1);
                        continue;
                    }
                    ...
                    ptrLTENR_GNBPHY phy = LTENR_GNBPHY_GET(gnbId, gnbInterfaceId);
                    if (phy == NULL) {
                        //fprintf(stderr, "No PHY data available for gNB Device %d Interface %d.\n", gnbId, gnbInterfaceId);
                        continue;
                    }

                    double actionValue_dB = actmsg->values[actionIndex];

                    fprintf(stderr, "Action value for gNB %d (Index %d) is %f dB\n", gnbId, actionIndex, actionValue_dB);
                    ...
                    while (assocInfo) {
                        if (assocInfo->isAssociated) {
                            for (UINT CAid = 0; CAid < phy->ca_count; CAid++) {
                                ...
                                fprintf(stderr, "Set power for gNB %d, CA %d, UE %d from %f dBm to %f dBm\n", gnbId, CAid, assocInfo->ueId, previousPower, newPower);
                            }
                        }
        ...
        fprintf(stderr, "Error: apply_power_action_to_all_gNBs called with non-power command (command_name: %d)\n", actmsg->command_name);
    }
}
    ...
    while (assocInfo)
    {
        if (assocInfo->ueId == ueId && assocInfo->isAssociated)
        {
            ...
            for (UINT CAid = 0; CAid < phy->ca_count; CAid++)
            {
                ...
                for (UINT i = 0; i < layerCount; i++)
                {
                    double throughput = (pdcp->TotalPDSCHBytes * 8) / (3 * frameDuration);
                    fprintf(stderr, "Layer %d: Throughput = %f bps\n", i, throughput);
    ...
    while (assocInfo)
    {
        if (assocInfo->ueId == ueId && assocInfo->isAssociated)
        {
            ...
            if (phy->ca_count > 0)
            {
                ...
                {
                    throughput_value = (pdcp->TotalPDSCHBytes * 8) / (3 * frameDuration);
                    fprintf(stderr, "Layer 0: Throughput = %f bps\n", throughput_value);
