...
void execute_CellLoad_command(ptrCLIENTINFO info, ptrCOMMANDARRAY command, int index)
{
    ...
    contain_obs(info, "CellLoadData", values, 1);
...
void execute_EdgeUE_command(ptrCLIENTINFO info, ptrCOMMANDARRAY command, int index)
{
    ...
    while (assocInfo) {
        if (assocInfo->isAssociated) {
            ...
            if (distance >= 525.0) { //edge ue = 70% upper
                edgeUECount++;
            }
        }
        assocInfo = LTENR_ASSOCIATEDUEPHYINFO_NEXT(assocInfo);
    ...
    fprintf(stderr, "Total connected UEs: %d\n", totalUECount);
    fprintf(stderr, "Edge UEs (>600 m): %d\n", edgeUECount);
    fprintf(stderr, "Percentage of Edge UEs (>600 m): %f%%\n", edgeUEPercentage);

    float values[1];
    values[0] = (float)edgeUEPercentage;

    contain_obs(info, "EdgeUEData", values, 1);
