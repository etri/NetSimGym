...
class NetSimEnv1(gym.Env):
    def __init__(self, env_config: Optional[dict] = None):
        ...
        self.State_list =self.env_config.get('State_list', ["SINR_ex1"])
        ...
        self.Action_list =self.env_config.get('Action_list', ["Power"])
        self.Reward_list =self.env_config.get('Reward_list', ["Sum_ex1"])
        ...
        self.obs_space_list = [
                               ...
                               for low, high, shape in self.env_config.get('obs_space_list', [[-20, 20, 6]])
        ]
        self.act_space_list = [
            [np.float32(low), np.float32(high), int(shape)]
            for low, high, shape in self.env_config.get('act_space_list', [[27, 46, 3]])
        ]
